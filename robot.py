...
class Robot():
	def __init__(self):
		...
		#self.astarPath = Astar(self.map_size, self.start, self.goal, self.move_list, self.pits, self.walls)
		#self.mdpPolicy = mdp();
		#self.mfmdp = mfmdp();
		...
		'''
		mdp_inst = PolicyList()
		mdp_inst.data = self.mdpPolicy
		
		#print(astar_inst.data)
		#print(mdp_inst.data)
		rospy.sleep(1)
		for i in self.astarPath:
			astar_inst = AStarPath()
			astar_inst.data = i
			self.astar_pub.publish(astar_inst)
		rospy.sleep(1)
		#rospy.sleep(3000)
		self.mdp_pub.publish(mdp_inst)
		rospy.sleep(1)
		'''
